...
function summarizeVariant(variant) {
    ...
    if (variant.suggesterModel)
        parts.push(`model: ${variant.suggesterModel}`);
    if (variant.suggesterThinking)
        parts.push(`thinking: ${variant.suggesterThinking}`);
    if (variant.maxSuggestionChars)
        parts.push(`chars: ${variant.maxSuggestionChars}`);
    if (variant.maxRecentUserPrompts)
        parts.push(`recent: ${variant.maxRecentUserPrompts}`);
    if (variant.maxRecentUserPromptChars)
        parts.push(`prompt chars: ${variant.maxRecentUserPromptChars}`);
    if (variant.maxChangedExamples)
        parts.push(`changed: ${variant.maxChangedExamples}`);
    if (variant.transcriptMaxContextPercent)
        parts.push(`steering ctx≤${variant.transcriptMaxContextPercent}%`);
    if (variant.transcriptMaxMessages)
        parts.push(`msgs≤${variant.transcriptMaxMessages}`);
    if (variant.transcriptMaxChars)
        parts.push(`chars≤${variant.transcriptMaxChars}`);
    if (variant.transcriptRolloutPercent !== undefined)
        parts.push(`rollout: ${variant.transcriptRolloutPercent}%`);
    return parts.join(" · ") || "inherits base settings";
}
async function promptPositiveIntOrClear(ctx, label, currentValue) {
...
async function promptVariantModel(ctx, currentValue) {
...
async function promptVariantThinking(ctx, currentValue) {
    const selected = await ctx.ui.select(`Suggester thinking (current: ${currentValue ?? "inherit"})`, ["(inherit)", ...THINKING_LEVELS, SESSION_DEFAULT]);
    if (!selected)
        return undefined;
    return selected === "(inherit)" ? null : selected;
}
async function promptVariantStrategy(ctx, currentValue) {
    const selected = await ctx.ui.select(`Suggester strategy (current: ${currentValue ?? "inherit"})`, ["(inherit)", "compact", "transcript-steering"]);
    if (!selected)
        return undefined;
    return selected === "(inherit)" ? null : selected;
}
async function editVariantUi(ctx, composition, variantName) {
    ...
    while (true) {
        ...
        if (action === "maxRecentUserPromptChars") {
            const next = await promptPositiveIntOrClear(ctx, `Recent user prompt chars for ${variantName} (blank = inherit)`, variant.maxRecentUserPromptChars);
            if (next === undefined)
                continue;
        ...
        if (action === "transcriptMaxContextPercent") {
            const next = await promptPositiveIntOrClear(ctx, `Transcript max context percent for ${variantName} (1-100, blank = inherit)`, variant.transcriptMaxContextPercent);
            if (next === undefined)
                continue;
        ...
        if (action === "transcriptMaxMessages") {
            const next = await promptPositiveIntOrClear(ctx, `Transcript max messages for ${variantName} (blank = inherit)`, variant.transcriptMaxMessages);
            if (next === undefined)
                continue;
        ...
        if (action === "transcriptMaxChars") {
            const next = await promptPositiveIntOrClear(ctx, `Transcript max chars for ${variantName} (blank = inherit)`, variant.transcriptMaxChars);
            if (next === undefined)
                continue;
        ...
        if (action === "transcriptRolloutPercent") {
            const next = await promptPositiveIntOrClear(ctx, `Transcript rollout percent for ${variantName} (1-100 or blank = inherit)`, variant.transcriptRolloutPercent);
            if (next === undefined)
                continue;
            if (next !== null && next > 100) {
                ctx.ui.notify("Value must be 1-100 or blank to inherit.", "error");
...
export async function manageVariantsUi(ctx, composition) {
    const store = composition.stores.variantStore;
    while (true) {
        const items = [
        ...
        if (!variantAction || variantAction === "Back")
            continue;
        ...
        if (variantAction === "Delete") {
            ...
            if (confirmed !== `delete ${entry.name}`)
                continue;
...
async function generateSuggestionForVariant(ctx, composition, variantName) {
    const turn = composition.runtimeRef.getLastTurnContext();
    if (!turn)
        throw new Error("No current suggestion context available yet. Wait for an assistant completion first.");
    ...
    const suggestionEngine = new SuggestionEngine({
        ...
        transcriptPromptContextBuilder: new TranscriptPromptContextBuilder(effectiveConfig, new PiSessionTranscriptProvider(composition.runtimeRef)),
    });
    const steering = {
...
async function chooseComparisonWinner(ctx, variantA, variantB, suggestionA, suggestionB) {
    const items = [
...
export async function runVariantComparison(ctx, composition, variantA, variantB) {
    if (variantA === variantB)
        throw new Error("Pick two different variants to compare.");
    if (!composition.stores.variantStore.getVariant(variantA))
        throw new Error(`Unknown variant: ${variantA}`);
    if (!composition.stores.variantStore.getVariant(variantB))
        throw new Error(`Unknown variant: ${variantB}`);
...
export async function showAbStats(ctx, composition) {
...
export async function handleVariantCommand(args, ctx, composition) {
...
export async function handleAbCommand(args, ctx, composition) {
    const tokens = args.trim().split(/\s+/).filter(Boolean);
    if (tokens[0] === "stats") {
...
export async function runAbTestingUi(ctx, composition) {
