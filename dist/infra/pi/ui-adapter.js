...
export function hasActiveUi(ctx) {
    if (!ctx)
        return false;
    try {
        return ctx.hasUI;
...
export function withActiveUi(ctx, action) {
    if (!ctx)
        return undefined;
    try {
        if (!ctx.hasUI)
...
export function notifyActiveUi(ctx, message, level) {
...
export function acceptWidgetSuggestion(runtime) {
    if (!usesWidgetSuggestion(runtime.suggestionDisplayMode))
        return "unavailable";
